<div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        <div class="flex">
            @if($todo->completed)
                <input wire:click="toggle({{$todo->id}})" type="checkbox" class="mr-4 relative top-1" checked>
            @else
                <input wire:click="toggle({{$todo->id}})" type="checkbox" class="mr-4 relative top-1">
            @endif
    ...
    <span class="text-xs text-gray-500">{{$todo->updated_at}}</span>
